  ...
  switch (step) {
    case 1:
      ...
      if (!formData.uniqueID.trim()) {
        errors.uniqueID = 'Company Unique ID is required';
      } else if (!/^\d+$/.test(formData.uniqueID.trim())) {
        errors.uniqueID = 'Company Unique ID must be a number';
      }
      if (!formData.uniqueIDAuthority || !formData.uniqueIDAuthority.trim()) {
        errors.uniqueIDAuthority = 'Unique ID Authority is required';
      } else if (!/^\d+$/.test(formData.uniqueIDAuthority.trim())) {
        errors.uniqueIDAuthority = 'Unique ID Authority must be a number';
      }
      ...
      if (!formData.authorizedcontactName || !formData.authorizedcontactName.trim()) {
        errors.authorizedcontactName = 'Authorized Contact Name is required';
      }
      
      if (!formData.authorizedemail || !formData.authorizedemail.trim()) {
        errors.authorizedemail = 'Authorized Email is required';
      } else if (!validateEmail(formData.authorizedemail)) {
        errors.authorizedemail = 'Please enter a valid email address';
      }
      
      if (!formData.authorizedphone || !formData.authorizedphone.trim()) {
        errors.authorizedphone = 'Authorized Phone number is required';
      } else if (!validatePhone(formData.authorizedphone)) {
        errors.authorizedphone = 'Please enter a valid phone number';
      }
      break;
